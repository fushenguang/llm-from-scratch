...
export class CausalSelfAttention {
  ...
  /**
   * Creates a masked multi-head self-attention module.
   *
   * @param config Attention dimensions and maximum context length.
  ...
  /**
   * Applies masked multi-head self-attention to a sequence of hidden states.
   *
  ...
  public apply(x: TensorLike): TensorLike {
    return this.tf.tidy(() => {
      ...
      const q = qkvSplit[0]; // shape: [B, T, C]
      const k = qkvSplit[1]; // shape: [B, T, C]
      const v = qkvSplit[2]; // shape: [B, T, C]
  ...
  /**
   * Returns the attention module's trainable variables.
   *
   * @returns Projection variables for qkv and output projection.
   */
