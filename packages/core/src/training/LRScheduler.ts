...
export function getLR(step: number, config: LRSchedulerConfig): number {
  ...
  if (config.warmupSteps > 0 && clampedStep < config.warmupSteps) {
    return config.maxLr * (step / config.warmupSteps);
  }

  const decaySpan = config.maxSteps - config.warmupSteps;
  const progress = Math.min(
    Math.max((clampedStep - config.warmupSteps) / decaySpan, 0),
    1
  );
