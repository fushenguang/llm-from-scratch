...
function setOptimizerLearningRate(optimizer: tf.Optimizer, learningRate: number): void {
  const setter = Reflect.get(optimizer as object, "setLearningRate");

  if (typeof setter === "function") {
    Reflect.apply(setter, optimizer, [learningRate]);
    return;
  }

  const currentLearningRate = Reflect.get(optimizer as object, "learningRate");
  if (
    currentLearningRate !== null &&
    typeof currentLearningRate === "object" &&
    typeof Reflect.get(currentLearningRate as object, "assign") === "function"
  ) {
    const scalar = tf.scalar(learningRate);
    Reflect.apply(
      Reflect.get(currentLearningRate as object, "assign") as (...args: unknown[]) => void,
      currentLearningRate,
      [scalar]
    );
    scalar.dispose();
  ...
  const updated = Reflect.set(optimizer as object, "learningRate", learningRate);
  ...
  public async *train(
    ...
    try {
      ...
      for (let step = 1; step <= config.maxSteps; step += 1) {
        ...
        const stepLearningRate = currentLearningRate;
        const { loss, nextLearningRate } = tf.tidy(() => {
          ...
          if (!appliesDecoupledWeightDecay) {
            applyDecoupledWeightDecay(trainableVariables, stepLearningRate, config.weightDecay);
          }
          optimizer.applyGradients(clippedGrads);
          const scheduledNextLearningRate =
            step < config.maxSteps
              ...
              : stepLearningRate;
          if (scheduledNextLearningRate !== stepLearningRate) {
            setOptimizerLearningRate(optimizer, scheduledNextLearningRate);
          }
          ...
          return {
            loss: lossValue,
            nextLearningRate: scheduledNextLearningRate
          };
        });
        ...
        currentLearningRate = nextLearningRate;
        latestTrainLoss = loss;
        yield { type: "step", step, loss, lr: stepLearningRate, tokensPerSec };
